...
class SeriesRepositoryImpl implements SeriesRepository {
  ...
  SeriesRepositoryImpl({required this.remoteDataSource, required this.localDataSource});
  ...
  Future<Either<Failure, String>> saveWatchlistSeries(SeriesDetail series) async {
    try {
      final result =
          await localDataSource.insertWatchlistSeries(SeriesTable.fromEntity(series));
      return Right(result);
  ...
  Future<Either<Failure, String>> removeWatchlistSeries(SeriesDetail series) async {
    try {
      final result =
          await localDataSource.removeWatchlistSeries(SeriesTable.fromEntity(series));
      return Right(result);
    } on DatabaseException catch (e) {
      return Left(DatabaseFailure(e.message));
    }
