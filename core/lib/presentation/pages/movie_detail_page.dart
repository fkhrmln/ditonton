...
class _MovieDetailPageState extends State<MovieDetailPage> {
  ...
  void initState() {
    super.initState();
    Future.microtask(() {
      context.read<MoviesDetailBloc>().add(OnFetchMoviesDetail(widget.id));
      context
          .read<MoviesRecommendationBloc>()
          .add(OnFetchMoviesRecommendation(widget.id));
      context.read<MoviesWatchlistCubit>().loadWatchlistStatus(widget.id);
    });
  }
    ...
    return Scaffold(
      body: BlocBuilder<MoviesDetailBloc, MoviesDetailState>(
        builder: (context, detail) {
          if (detail is MoviesDetailLoading) {
            return Center(
              child: CircularProgressIndicator(),
            );
          } else if (detail is MoviesDetailHasData) {
            final movie = detail.result;

            return BlocBuilder<MoviesRecommendationBloc, MoviesRecommendationState>(
              builder: (context, recommendation) {
                if (recommendation is MoviesRecommendationLoading) {
                  return Center(
                    child: CircularProgressIndicator(),
                  );
                } else if (recommendation is MoviesRecommendationHasData) {
                  return BlocBuilder<MoviesWatchlistCubit, MoviesWatchlistState>(
                    builder: (context, state) {
                      if (state is MoviesWatchlistIsAdedd) {
                        return SafeArea(
                          child: DetailContent(
                            movie,
                ...
                } else if (recommendation is MoviesRecommendationError) {
                  return Center(
          ...
          } else if (detail is MoviesDetailError) {
            return Center(
                          ...
                          children: [
                            ...
                            ElevatedButton(
                              onPressed: () async {
                                ...
                                final message =
                                    context.read<MoviesWatchlistCubit>().message;
